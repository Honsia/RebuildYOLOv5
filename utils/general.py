...
def compute_loss(p, targets, model):  # predictions, targets, model
    ...
    s = 3 / np  # v5s31_mask count scaling
    lbox *= h['box'] * s
...
def coco_single_class_labels(path='../coco/labels/train2014/', label_class=43):
    ...
    if os.path.exists('new/'):
        shutil.rmtree('new/')  # delete v5s31_mask folder
    os.makedirs('new/')  # make new v5s31_mask folder
...
def output_to_target(output, width, height):
    # Convert model v5s31_mask to target format [batch_id, class_id, x, y, w, h, conf]
